...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async (location: string, useAI: boolean = true) => {
    ...
    try {
      if (useAI) {
        ...
        // Background fetch for AI insight
        getAIWeatherInsight(data.weather).then(insight => {
          setState(prev => ({ ...prev, aiInsight: insight }));
        }).catch(() => {
          setState(prev => ({ ...prev, aiInsight: "Atmospheric complexity detected. Monitor for updates." }));
        });
      } else {
        ...
        setState({
          ...
          sources: [],
          error: null,
          aiInsight: "Analysis indicates standard seasonal patterns for this coordinate."
        });
      }
    } catch (err: any) {
      ...
      setState(prev => ({ 
        ...prev, 
        loading: false, 
        error: err.message || `Unable to locate "${location}". Please refine your search or try a major global hub.` 
      }));
    }
  }, []);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          setState(prev => ({ 
            ...prev, 
            error: "Geolocation access denied. Please manually search for your city." 
          }));
        }
      );
    }
  ...
  if (state.loading && !state.weather) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-[#0B0C10] text-white">
        ...
        <h1 className="text-2xl font-black text-white tracking-[0.2em] uppercase mb-2">SkyCast AI</h1>
        <p className="text-gray-500 text-sm font-medium tracking-wide">Syncing satellite surveillance grid...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-screen bg-[#0B0C10] text-gray-100 overflow-hidden">
      <Sidebar />

      <main className="flex-1 h-screen overflow-y-auto p-4 lg:p-10 scrollbar-hide">
        <header className="flex flex-col md:flex-row gap-6 items-center justify-between mb-10 max-w-7xl mx-auto w-full">
          <div className="flex flex-col">
            ...
            <p className="text-gray-500 text-xs mt-1">Global atmospheric intelligence dashboard.</p>
          </div>
        ...
        <div className="max-w-7xl mx-auto w-full">
          ...
          <div className="flex items-center gap-4 mb-10 overflow-x-auto pb-4 scrollbar-hide">
            <div className="flex items-center gap-2 text-gray-500 text-xs font-bold uppercase tracking-widest whitespace-nowrap">
              <Globe size={14} /> Major Hubs:
            </div>
            {FEATURED_CITIES.map(city => (
              <button 
                key={city}
                onClick={() => fetchData(city, true)}
                className={`px-4 py-2 rounded-xl border text-xs font-medium transition-all whitespace-nowrap ${
                  weather?.city.toLowerCase().includes(city.toLowerCase()) 
                    ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-500/20' 
                    : 'bg-[#1A1C23] border-gray-800 text-gray-400 hover:border-blue-500 hover:text-white'
                }`}
              >
                {city}
              </button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-4 flex flex-col gap-10">
              ...
              <div className="glass-card p-8 border-blue-500/10 bg-gradient-to-br from-[#1E202C] to-[#0B0C10] shadow-xl">
                ...
                <p className="text-gray-200 text-sm leading-relaxed">{aiInsight || "Parsing atmospheric telemetry..."}</p>
              </div>
            ...
            <div className="lg:col-span-8 flex flex-col gap-10">
              ...
              <div className="glass-card p-10 border-white/5 shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between mb-10">
                  <div className="flex flex-col">
                    <h2 className="text-xl font-black text-white uppercase tracking-widest">Temperature Cycle</h2>
                    <p className="text-gray-500 text-xs font-bold uppercase mt-1">24 Hour Forecast Data</p>
                  </div>
                  <div className="bg-blue-600/10 text-blue-500 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">LIVE SENSOR FEED</div>
                </div>
              ...
              <div className="glass-card p-8 border-white/5 shadow-xl">
                <h2 className="text-lg font-black text-white uppercase tracking-widest mb-8 border-b border-white/5 pb-4">7-Day Extended Projections</h2>
                  ...
                  {daily.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 rounded-2xl hover:bg-white/[0.03] transition-all group">
                      ...
                      <div className="flex items-center gap-10">
                        <span className="text-gray-500 text-xs hidden sm:block italic">{item.description}</span>
        ...
        <footer className="mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6 pb-12 max-w-7xl mx-auto w-full">
          <p className="text-gray-600 text-xs font-bold uppercase tracking-widest">© SkyCast AI Global Surveillance Network</p>
          <div className="flex gap-8">
            {['Privacy Protocol', 'Node Status', 'GCP Compliance', 'AI Policy'].map(link => (
